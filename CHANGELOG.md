...
- **Profiles, Presence, Receipts, Typing, and Account Data**: Implemented issue #8 — presence get/set with the typed `PresenceStatus`/`Presence` models (online/offline/unavailable, optional status message, `lastActiveAgo`/`currentlyActive` parsing), read receipts (`m.read`, `m.read.private`) with optional thread identifiers and fully-read markers via a typed `ReadMarkers` builder, typing notifications with optional timeout, and global/room account data read-write with a generic `JsonValue` escape hatch preserving unknown event types (`M_NOT_FOUND` maps to empty) (#25)
