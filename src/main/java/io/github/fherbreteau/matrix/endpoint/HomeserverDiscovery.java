...
/**
 * Homeserver discovery per the Matrix specification: resolves the
 * authoritative homeserver URL from {@code /.well-known/matrix/client},
 * with clear fallback rules — any discovery failure (transport error,
...
public final class HomeserverDiscovery {
    ...
    /**
     * Discovers the homeserver behind {@code baseUrl}. The returned record
     * carries the validated homeserver URL, an optional identity server
     * URL and the raw well-known payload; {@link DiscoveredHomeserver#usedFallback()}
     * tells whether the explicit URL was used instead of the discovery
     * result.
     */
    public static DiscoveredHomeserver discover(HttpTransport transport, String baseUrl) {
        ...
        return new DiscoveredHomeserver(homeserverUrl, extractUrl(wellKnown, "m.identity_server"), wellKnown, false);
    }

    /**
     * Normalizes a homeserver base URL: removes trailing slashes and
     * validates the scheme (http or https).
     */
