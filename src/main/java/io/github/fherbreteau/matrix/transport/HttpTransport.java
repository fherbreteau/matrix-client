...
public interface HttpTransport {
    ...
    /**
     * Sends the request and returns the response. Implementations must never
     * log or expose secrets (access tokens, credentials).
     */
    Response send(Request request);

    /**
     * An HTTP request. The {@code toString()} representation redacts the
     * {@code Authorization} header so access tokens never leak into logs.
     */
    ...
    /**
     * An HTTP response with status code, response headers and decoded body.
     * The {@code retryAfterMs} field carries a parsed {@code Retry-After}
