...
public final class JdkHttpTransport implements HttpTransport {
    ...
    /**
     * Returns a builder producing a {@link HttpTransportConfig}; use it to
     * create a transport with {@code new JdkHttpTransport(config.build())}.
     */
    ...
    public Response send(Request request) {
        ...
        try {
            ...
            throw new TransportTimeoutException("HTTP request timed out: " + request.method()
                    + " " + request.url(), e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new TransportInterruptedException("HTTP request interrupted", e);
        } catch (IOException e) {
            throw new UncheckedTransportException("HTTP request failed: " + request.method()
                    + " " + request.url(), e);
        }
    }
